...
setup() {
  ...
  sudo wget https://raw.githubusercontent.com/janmasarik/resolvers/master/resolvers.txt -O $HOME/.config/subzero/resolvers.txt
...
while getopts ":d:o:w:a:hp" opt; do
  case ${opt} in
    d)
      domainName="${OPTARG}"
      ;;
...
subDomains() {
  ...
  local virustotal=$(cat $HOME/.config/subzero/config.json | jq '.API.virustotal')
  local securitytrails=$(cat $HOME/.config/subzero/config.json | jq '.API.securitytrails')
  local gitlab=$(cat $HOME/.config/subzero/config.json | jq '.API.gitlab')
  local dnsResolver="$HOME/.config/subzero/resolvers.txt"
  ...
  if [ -n "$sublist" ]; then
    ...
    shuffledns -d $domainName -w $sublist -r $dnsResolver -o shuffledns-output.txt -m $(which massdns) -mode bruteforce -silent 1>/dev/null
  fi
  ...
  if [ "$httpxProbe" ]; then
    ...
    httpx -r $dnsResolver -o "$fn-httpx.txt" -l "$fn.txt" -silent -sc -title -server -cname -cdn -screenshot -system-chrome 1>/dev/null
  fi
