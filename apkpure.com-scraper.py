...
def main():
    ...
    parser.add_argument('-xapk', action='store_false', default=True, help="Ignore xapk files in download.")
    ...
    def _download_apk(url, filename):
        ...
        if os.path.exists(os.path.join(final_directory, pathlib.Path(filename).stem + ".apk")) or os.path.exists(os.path.join(final_directory, pathlib.Path(filename).stem + ".xapk")):
            console.print("[yellow]... duplicate " + filename + " [/yellow]")
            return False
